...
#scrape_boxscore(home_str, away_str, date_str, home, opp, int(year))

def scrape_boxscore(home_str, away_str, date_str, home, opp_full, year):

	if year > 2015:
		raise Exception('Not working for 2015 yet! Defense tables changed')
	# print home_team_full
...
def scrape_gamelog(team, year):
	...
	first_row = True
	for row in table_gamelog.find_all("tr"):
...
def create_row_data(home_team, away_team, Week, Year, Model):
	if len(home_team) != 3:
		home_str = team_rename(home_team)
	else:
		home_str = home_team
	if len(away_team) != 3:
	...
	if not os.path.isfile('../NFL/Data/game_data_' + home_str + '_' + Year + '.json'):
		print 'Webscraping game log for home team (with a short delay)'
		scrape_gamelog(home_str, Year)
		sleep(randint(80,150) / 7 / pi)
	if not os.path.isfile('../NFL/Data/game_data_' + away_str + '_' + Year + '.json'):
		print 'Webscraping game log for away team (with a short delay)'
		scrape_gamelog(away_str, Year)
		sleep(randint(80,150) / 7 / pi)

	home_file = open('../NFL/Data/game_data_' + home_str + '_' + Year + '.json','r')
	home_game_data = json.load(home_file)
	away_file = open('../NFL/Data//game_data_' + away_str + '_' + Year + '.json','r')
	away_game_data = json.load(away_file)

	# print 'home: ', home_game_data
	# print 'away: ', away_game_data
	...
	for category in Model:
		if category == 'WinP':
			row_data.append(get_win_percentages(home_game_data, Week, True)[0])
		elif category == 'LocWinP':
			row_data.append(get_win_percentages(home_game_data, Week, True)[1])
		else:
			row_data.append(colsum(home_game_data[category], home_game_data['Week'], Week) / float(home_total_games_played))
	
	#Now add away data
	for category in Model:
		if category == 'WinP':
			row_data.append(get_win_percentages(away_game_data, Week, False)[0])
		elif category == 'LocWinP':
			row_data.append(get_win_percentages(away_game_data, Week, False)[1])
		else:
			row_data.append(colsum(away_game_data[category], away_game_data['Week'], Week) / float(away_total_games_played))
	print row_data
	home_file.close()
	away_file.close()
	return row_data

def create_row_data_train(home_game_data, away_team, Week, year, Model, measure):
...
# print create_row_data('crd', 'atl', 12, '2014', Model)

model_file = '../NFL/Models/Master_file_11_14.csv'
				...
				for week_num in range(start_week[count], end_week[count] + 1):
					if week_num in team_data['Week'] and team_data['Home'][team_data['Week'].index(week_num)]: #if not a bye week and a home game
						...
						if team_data['W/L'][week_index] == 'W':
							...
							result = 0
						row_data_single = create_row_data_train(team_data, away_team_name, week_num, str(year), master_variable_list, 'single game')
						row_data_avg_ytd = create_row_data_train(team_data, away_team_name, week_num, str(year), master_variable_list_avg_ytd, 'avg_ytd')
						row_data_avg_yr = create_row_data_train(team_data, away_team_name, week_num, str(year), master_variable_list_avg_yr, 'avg_yr')
						...
						# wr.writerow([str(year), str(week_num), team, team_rename(away_team_name), int(team_data['W/L'][week_index] == 'W'), vegas_index, over_under_index] + row_data_avg_ytd)
